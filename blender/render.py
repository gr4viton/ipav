...
def createCamera(origin, target):
    ...
    bpy.ops.object.add(type="CAMERA", location=origin, rotation=(pi / 2, 0, pi))
    ob = bpy.context.object
    ob.name = "MyCamOb"
    cam = ob.data
    cam.name = "MyCam"
...
def run(origin):
    ...
    bpy.ops.object.add(type="EMPTY", location=midpoint),
...
def render():
    ...
    path = os.path.abspath(
        "".join(["D:/DEV/PYTHON/pyCV/kivyCV_start/blender/pic/", str(i), "image.jpg"])
    )
