...
class RealCamera:
    ...
    def create_cone(self):
        new_cone = bpy.ops.mesh.primitive_cone_add(
            vertices=4, radius1=2, depth=10.0, location=self.pos, rotation=self.rot
        )
    ...
    def duplicate(self, obj, duplicate_name, alpha=1):
        ...
        if alpha < 1:
            ...
            if self.blue_mat is None:
                self.blue_mat = self.makeMaterial(
                    "BlueSemi", (0, 0, 1), (0.5, 0.5, 0), 0.5
                )
    ...
    def get_contour_projection_object(self):
        ...
        faces_canvas = [
            (x + 1, x, start_index) for x in range(start_index + 1, hull_max_index)
        ]
